...
(schema/defn ^:always-validate
  ...
  [{:keys [keystore-config client-auth ssl-crl-path cipher-suites protocols disallow-renegotiation]}
    ...
    (when-let [https (:https config)]
      (let [ssl-ctxt-factory (ssl-context-factory
                               {:keystore-config (:keystore-config https)
                                ...
                                :disallow-renegotiation (:disallow-renegotiation https)
